        ...
        public override int UpdateResultsTable(DataTable table, string raceName, int heatCount)
        {
            ...
            try
            {
                using (var transaction = SqliteConn.BeginTransaction())
                {
                    ...
                    {
                        var type = table.Columns[i].DataType.ToString() switch
                        {
                            "System.Int64" => SqliteType.Integer,
                            "System.String" => SqliteType.Text,
                    ...
                    {
                        for (int i = 0; i < table.Columns.Count; i++) command.Parameters[i].Value = row[i];
